...
function Ambulance() {
  ...
  const accepterAlerte = (alerte) => {
    ...
    const temps = Math.round(distance / 0.5);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>🚑 Centre SAMU — 185</h1>
      ...
      <div style={styles.content}>
        <h2 style={styles.sectionTitle}>🚨 Accidents & urgences ({alertes.length})</h2>
        {alertes.length === 0 ? (
          <div style={styles.empty}>
            <p style={styles.emptyText}>✅ Aucune urgence active</p>
            <p style={styles.emptyHint}>Les accidents apparaîtront ici en temps réel</p>
          </div>
        ) : (
          alertes.map((alerte, i) => (
            <div key={i} style={styles.alerteCard}>
              ...
              <button
                ...
                {enRoute ? '🚑 Déjà en intervention' : '🚑 Envoyer ambulance'}
              </button>
            </div>
          ))
        )}
...
const styles = {
  ...
  tempsNumber: { color: '#c62828', fontSize: '48px', fontWeight: 'bold', lineHeight: 1 },
  tempsLabel: { color: '#666', fontSize: '14px' },
  arrive: { color: '#4caf50', fontSize: '20px', fontWeight: 'bold' },
  annulerBtn: { padding: '10px 24px', backgroundColor: '#fff', color: '#c62828', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' },
  ...
  alerteCard: { backgroundColor: '#16213e', borderRadius: '12px', padding: '20px', marginBottom: '16px', border: '2px solid #c62828' },
  alerteHeader: { display: 'flex', gap: '16px', marginBottom: '16px' },
  alerteIcon: { fontSize: '40px' },
  alerteImmat: { color: '#c62828', margin: '0 0 4px 0', fontSize: '20px' },
  ...
  intervenirBtn: { width: '100%', padding: '12px', backgroundColor: '#c62828', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '16px', cursor: 'pointer', fontWeight: 'bold' },
};
